...
4. Finish with: test run output, `pip-audit`/`npm audit` output, a `LICENSE` (MIT), and a README section "What is and isn't stored" that a non-technical reviewer can understand. Sample data, screenshots and fixtures use fictional names only (e.g. `example.org`, "Integration PROD").